...
public class AIAgent : MonoBehaviour
{
    ...
    private void AIMovementInit()
    {
        ...
        (flock, flee, seek, pursue, obstacleAvoidance, lookWryGo, wander, aStarPathFinding, turnTowardTarget, turnAwayFromTarget, flocking) =
            (movementsDic.GetValueOrDefault(typeof(Flock)) as Flock,
             ...
             movementsDic.GetValueOrDefault(typeof(Astar)) as Astar,
    ...
    private List<AIMovement> MoveTowardsTarget()
    {
        ...
        if (actualSpeed <= slowSpeedThreshold)
        { // 1. If the agent is moving slowly
            if (distanceToTarget < nearRadius && !target.CompareTag("Token"))
            { // 1.1. If the target is within the nearRadius (and not a token)
                applyMovement.Add(seek);
            }
            else if (isTurnOnSpot())
            { // 1.2.1  If the agent is not facing the target (and there is no obstacle between), stop and turn towards the target
                StopMoving();
        ...
        { // 2. If the agent is moving fast
            if (CheckInCone())
            { // 2.1 If the target is within the cone of view
                applyMovement.Add(pursue);
            }
            else if (isTurnOnSpot())
            { // 2.2.1 If the agent is not facing the target (and there is no obstacle between), stop and turn towards the target
                StopMoving();
            }
            else
            { // 2.2.2 setting off once more
    ...
    private List<AIMovement> MoveAwayFromTarget()
    {
        ...
        if (distanceToTarget > farRadius)
        { // added: if target is out of farRadius
            applyMovement.Add(wander);
        }
        else
        { //if target is in the farRadius
    ...
    public void TargetReset(String tagName = "Seeker")
    {
        ...
        foreach (GameObject tgt in targets)
        {
            ...
            if (aiAgent == null || aiAgent.isBeingChased)
            { // Check if target has AIAgent component and is not already being chased
    ...
    {
        if (gameObject.tag != null && gameObject.tag == "Chaser")
        { // If agent is chaser, set target for A* pathfinding to be the current target
            aStarTarget = target;
        }
        else if (gameAgent.isSomeOneFrozen)
        {
    ...
    private void OnCollisionEnter(Collision collision)
    {
        ...
        if (CompareTag("Frozen") && collision.gameObject.tag == "Seeker")
        {
