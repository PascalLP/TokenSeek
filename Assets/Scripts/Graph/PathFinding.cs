...
public class PathFinding : MonoBehaviour
{
    ...
    private float MovementCost(GraphNode next, GraphNode current, Graph graph) {
        Vector3 movementOffset= (next.transform.position - current.transform.position) / graph.gridCellSize;
    ...
    public List<GraphNode> FindPath(GraphNode start, GraphNode goal, bool isDebug, Heuristic heuristic = null, bool isAdmissible = true ) {
        if(graph == null) { return new List<GraphNode>(); }
        ...
        while (openList.Count > 0) {
            ...
            //if we found a entry that has smaller g value than gGoal, then it means that there is a shorter path, we have to search
                ...
                foreach (GraphNode entry in openList) {
            ...
            foreach (GraphNode next in neighbors) {
    ...
    private void FakePQListInsert(List<GraphNode> pqList, Dictionary<GraphNode, float> fnDict, GraphNode node) {
        if (pqList.Count == 0)
            pqList.Add(node);
        else {
